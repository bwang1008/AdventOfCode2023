...
def get_neighbors(curr_node: Node) -> List[Node]:
    ...
    return [
        ...
        if neighbor.num_same_direction <= 3
        and abs(neighbor.direction - curr_node.direction) != 2
    ]


def dijkstra(
    heat_loss: List[List[int]], source: Tuple[int, int], dest: Tuple[int, int]
) -> int:
    R: int = len(heat_loss)
    ...
    begin_node: Node = Node(source[0], source[1], UP, 0)
    dist[begin_node] = 0
    ...
    while not pq.empty():
        ...
        if curr_node.row == dest_row and curr_node.col == dest_col:
            last_node = curr_node
            break
...
def main() -> None:
    ...
    answer: int = dijkstra(
        heat_loss, (0, 0), (len(heat_loss) - 1, len(heat_loss[0]) - 1)
    )
    logger.info(f"{answer=}")  # 1256
